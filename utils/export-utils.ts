...
// Logo functionality removed for better performance and cleaner layout
...
export function exportToExcel(
  ...
  try {
    ...
    // Create cover sheet with logo placeholder
    const coverData = [
      ['PERSONAL BANKER'],
      ['INVESTMENT PORTFOLIO REPORT'],
      [''],
      ['Investor Name:', investorName || 'Not Available'],
      ...
      ['currency distribution, and investment type analysis.'],
      [''],
      ['Generated by Personal Banker - Preserve. Prosper. Reserve.'],
    ];
    ...
    coverWs['!merges'].push({ s: { r: 0, c: 0 }, e: { r: 0, c: 1 } });
    coverWs['!merges'].push({ s: { r: 1, c: 0 }, e: { r: 1, c: 1 } });
    ...
    if (currencyTotals && monthlyReturns) {
      const summaryData = [
        ['PORTFOLIO SUMMARY'],
        [''],
        ['Investor:', investorName || 'Not Available'],
...
export function exportToPDF(
  ...
  try {
    ...
    console.log('Exporting PDF with investor name:', investorName);
    ...
    doc.text('PERSONAL BANKER', 14, 12);

    // Add tagline
    doc.setFontSize(8);
    doc.setFont('helvetica', 'normal');
    doc.text('Preserve. Prosper. Reserve.', 14, 17);

    // Add main title
    doc.setFontSize(24);
    doc.setFont('helvetica', 'bold');
    doc.text('INVESTMENT PORTFOLIO REPORT', pageWidth / 2, 20, {
      align: 'center',
    });
    ...
    const displayName =
      investorName && investorName.trim() !== ''
        ? investorName
        : 'Not Available';
    console.log('Display name in PDF:', displayName);
    doc.text(`Investor: ${displayName}`, 14, 28);
    doc.text(`Report Date: ${formatDate(new Date())}`, pageWidth - 14, 28, {
      align: 'right',
    });
    doc.text(`Total Investments: ${investments.length}`, 14, 33);
    ...
    // Create table with improved styling and better column distribution
    autoTable(doc, {
      startY: startY + 5,
      head: [
        [
      ...
      columnStyles: {
        0: { cellWidth: 'auto' }, // Organisation - flexible
        1: { cellWidth: 'auto' }, // Type - flexible
        2: { cellWidth: 18, halign: 'center' }, // Currency
        3: { cellWidth: 'auto', halign: 'right' }, // Amount
        4: { cellWidth: 20, halign: 'center' }, // Rate
        5: { cellWidth: 'auto', halign: 'right' }, // Monthly
        6: { cellWidth: 'auto' }, // Maturity
        7: { cellWidth: 20, halign: 'center' }, // Days
      },
      margin: { left: 14, right: 14 },
      tableWidth: 'auto', // This ensures the table spans the full width
      didDrawCell: (data) => {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      // Generated by with branding
      doc.text(
        `Generated by Personal Banker | ${formatDate(new Date())}`,
        pageWidth - 14,
        pageHeight - 10,
        { align: 'right' }
      );
